...
export function findHostWindow(pid = process.pid, { guess = true } = {}) {
  ...
  // ancestor's console and following its owners finds the right window. The pseudoconsole window
  // itself is never recorded: Windows Terminal mirrors show/hide on it onto the whole terminal
  // window, so Return poking it made the terminal vanish. Processes without a visible console owner
  // (VS Code, the Claude desktop app) fall back to the first ancestor that owns a top-level window.
  const script = `
Add-Type -Namespace L -Name C -MemberDefinition '[DllImport("kernel32.dll")] public static extern bool FreeConsole(); [DllImport("kernel32.dll")] public static extern bool AttachConsole(uint p); [DllImport("kernel32.dll")] public static extern IntPtr GetConsoleWindow(); [DllImport("user32.dll")] public static extern IntPtr GetWindow(IntPtr h, uint c); [DllImport("user32.dll", CharSet=CharSet.Unicode)] public static extern int GetClassName(IntPtr h, System.Text.StringBuilder s, int n); [DllImport("user32.dll")] public static extern bool IsWindowVisible(IntPtr h); [DllImport("user32.dll")] public static extern uint GetWindowThreadProcessId(IntPtr h, out uint p); [DllImport("user32.dll", CharSet=CharSet.Unicode)] public static extern int GetWindowText(IntPtr h, System.Text.StringBuilder s, int n);'
...
while ($id -gt 4 -and $hops -lt 14) {
  ...
  if ([L.C]::AttachConsole([uint32]$id)) {
    ...
    if ($cw -ne [IntPtr]::Zero) {
      $root = $cw; $n = 0; while ($n -lt 8) { $o = [L.C]::GetWindow($root, 4); if ($o -eq [IntPtr]::Zero) { break }; $root = $o; $n++ }
      $cls = New-Object System.Text.StringBuilder 64; [void][L.C]::GetClassName($root, $cls, 64)
      if ($cls.ToString() -ne 'PseudoConsoleWindow' -and [L.C]::IsWindowVisible($root)) { Say $root; exit 0 }
    }
  }
