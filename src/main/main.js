 ...
 * Windows Terminal mirrors show, hide and minimize of a tab's pseudoconsole window onto the whole
 * terminal window, so that window is never touched: a recorded one (hosts recorded before 0.6.7)
 * stands for the terminal window that owns it. For the same reason the helper leaves any console it
 * started with and is not run with -WindowStyle Hidden, which hides the console it is attached to,
 * and a window already on screen is not shown again.
 */
function focusHwnd(hwnd, owner = '') {
  ...
  const script = `Add-Type -Namespace L -Name W -MemberDefinition '${FOCUS_DEFS}'; [void][L.W]::FreeConsole(); $h=[IntPtr]::new([Int64]${hwnd}); if(-not [L.W]::IsWindow($h)){ 'gone'; exit 0 }; ${check} $cls=New-Object System.Text.StringBuilder 64; [void][L.W]::GetClassName($h,$cls,64); if($cls.ToString() -eq 'PseudoConsoleWindow'){ $n=0; while($n -lt 8){ $o=[L.W]::GetWindow($h,4); if($o -eq [IntPtr]::Zero){ break }; $h=$o; $n++ }; [void]$cls.Clear(); [void][L.W]::GetClassName($h,$cls,64); if($cls.ToString() -eq 'PseudoConsoleWindow'){ 'unknown'; exit 0 } }; if(-not [L.W]::IsWindowVisible($h) -and -not [L.W]::IsIconic($h) -and $cls.ToString() -eq 'CASCADIA_HOSTING_WINDOW_CLASS'){ 'gone'; exit 0 }; if([L.W]::IsIconic($h)){ [void][L.W]::ShowWindow($h,9) } elseif(-not [L.W]::IsWindowVisible($h)){ [void][L.W]::ShowWindow($h,5) }; if([L.W]::GetForegroundWindow() -eq $h){ 'ok'; exit 0 }; $fg=[L.W]::GetForegroundWindow(); $ft=[L.W]::GetWindowThreadProcessId($fg,[IntPtr]::Zero); $ct=[L.W]::GetCurrentThreadId(); $a=[L.W]::AttachThreadInput($ft,$ct,$true); [void][L.W]::BringWindowToTop($h); $ok=[L.W]::SetForegroundWindow($h); if($a){ [void][L.W]::AttachThreadInput($ft,$ct,$false) }; if(-not $ok){ [L.W]::keybd_event(0x12,0,0,[UIntPtr]::Zero); [L.W]::keybd_event(0x12,0,2,[UIntPtr]::Zero); [void][L.W]::BringWindowToTop($h); $ok=[L.W]::SetForegroundWindow($h) }; Start-Sleep -Milliseconds 150; if([L.W]::GetForegroundWindow() -eq $h){ 'ok' } else { 'refused' }`;
  return new Promise((resolve) => {
    let out = '';
    const child = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', script], { windowsHide: true, stdio: ['ignore', 'pipe', 'ignore'] });
