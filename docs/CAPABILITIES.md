...
- **Return, 0.6.1 fixes.** Windows Terminal runs all its windows in one process, so the process's main window was often a different window, or one already closed (Terminal keeps a closed window's handle alive but hidden, and Return used to show it again). The hook now attaches to each ancestor's console and takes the root owner of the pseudoconsole window, which is the terminal window hosting that tab; measured from a second Terminal window, it found that window while the process-wide handle read 0. A hidden Terminal window now counts as closed, success means the window is really in front afterwards (measured: Notepad in front, Return brought the terminal back and reported ok), and Return no longer marks the agent seen. macOS records the app bundle the agent runs under (walking the process tree) instead of whatever was frontmost; not run on a Mac in this session.
- **Return, 0.6.2 fixes.** On macOS, a session whose process tree reaches no app bundle fell back to whatever app was frontmost, and every later `SessionStart` (resume, clear) did so again, so Return could open Slack. The common causes are now handled from the tree: iTerm2 runs shells under a detached `iTermServer` (parent launchd), which now stands for iTerm2, and inside tmux the walk starts at the tmux client, which sits in the terminal. The frontmost guess is only taken on a fresh start, is marked `via: frontmost`, and never replaces a window found from the tree. The tab's tty is recorded too; Return selects that tab in Terminal or iTerm2 (Automation permission, asked once; if refused, the app alone comes forward). On Windows, a recorded handle now owned by a different process (handles are reused after a window closes) counts as closed instead of bringing that app forward. Unit-tested against sample `ps` output; not run on a Mac in this session. A session that recorded a wrong app before 0.6.2 keeps it until it is resumed or restarted.
- **Conversation titles** (0.6.2): the Tracker row and the workspace thread card show the agent's own name for the conversation, the one its sidebar or resume list shows, and fall back to the latest prompt. Claude Code: the last `custom-title` (`/rename`) or `ai-title` line of the transcript, which it re-appends every few messages (measured on Claude Code 2.1.280: every ~50–170 KB), so the hook reads the last megabyte and then the first. Codex: the last `thread_name` for the thread in `$CODEX_HOME/session_index.jsonl` (verified with the 0.156 app server's `thread/name/set`). Whether the Codex desktop app's automatic titles go through that call was not verified; a thread it names only in its own storage keeps showing its prompt. A title appears after the turn that produced it ends.
...
- **Codex before 0.125 runs no hooks** unless `~/.codex/config.toml` has `[features] codex_hooks = true` (0.6.1). Measured with Codex 0.120: no hook fired without it; with it, the Layover hook recorded the session and the turn. 0.125 and later default hooks on and accept the key, so setup always writes it (one marked line under `[features]`, removed on Disconnect). An explicit `hooks = false` under `[features]` still wins; Settings says so.
